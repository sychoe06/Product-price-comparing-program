...
def currency(number):
    return f"${number:,.3f} per {main_unit}"
...
def display_details(products_list, products_dict, within_or_above):
    ...
    if len(products_list) == 0:  # Then tell the user this.
        ...
        # so that they have $'s and the per unit (e.g. $0.253 per g)
...
# !! Welcome screen !!
print("- " * 33)  # decoration
print(">------->> WELCOME TO THE PRODUCT PRICE COMPARING PROGRAM <<-------<")
print("- " * 33)  # decoration

# Ask user if they have used the program before and if not, show instructions
instructions = check_valid("Do you want the instructions? (Y/N): ", yes_no)
if instructions == "y":
    print("\n\t\t\t\t- - - - - Instructions - - - - -"
...
print("\n>> Product Price Comparing Program is now starting <<")
...
while name != EXIT_SIGN:
    ...
    if name == EXIT_SIGN:
        ...
        # Calculate unit price - Formats to 3dp
        unit_price = "{:.3f}".format(price / new_weight)
...
# The price list must not be empty for the average unit price to be calculated
if len(total_prices_list) > 0:
